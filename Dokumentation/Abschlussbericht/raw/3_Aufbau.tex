...
Ein Arduino Micro steuert den Oktokommander mittels eines I2C Bus an. Der I2C Bus benötgt lediglich 2 Pins auf dem Arduino, SDA für die eigentliche Datenverbindung und SCL für den vom Arduino vorgegebenen Takt. Jeder Sensor wird über eine, im System, einzigartige Hardware Adresse (I2C-Adressen) angesteuert.
Der Oktokommander erzeugt mittels eines I2C Expanders je 4 I2C Adressen die für die Zuordnung und Ansteuerung der einzelenen Verstärker benötigt werden.
...
\end{align*}
Hierbei steht $GBP$ für \textit{Gain Bandwidth Product}. Sie ist die Frequenz, bei der keine Verstärkung mehr stattfindet. Sie ist somit die Obergrenze für einen praktikablen Einsatz des Systems Für den Operationsverstärker LM358 ist das $GBP$ mit $\unit[1]{MHz}$ im Datenblatt angegeben. $C_I$ ist die Summe der beiden Sperrschichtkapazitäten der Photodiode und des Operationsverstärkers. Typische Anwendungsgrößen für $R_1$ liegen bei etwa $\unit[1]{M\Omega}$. Der Arbeitsbereich des Verstärkersystems beschränkt, durch aktive Bauelemente, sich auf einen bestimmten Frequenzbereich. Dieser lässt sich wie folgt berechnen:
\begin{align*}
	f_{\unit[3]{dB}} = \sqrt{\frac{GBP}{2\pi \cdot R_1 \cdot C_I}}
\end{align*}
Für unser System liegt die Bandbreite bei etwa $\unit[73]{kHz}$. Dies ist auch die Grenzfrequenz des Verstärkersystems. Die Grenzfrequenz ist die Frequenz bei derdie Verstärkung auf den $\frac{1}{\sqrt{2}}$ fachen Wert der maximalen Verstärkung gesunken ist und bei der die hälfte der maximal Leistung an einen, rein Ohm'schen, Verbraucher abgegeben wird.
